...
import { z } from 'zod'
import { UserPlus, Landmark, X, Check, Phone, User, MapPin, Sparkles, Loader2 } from 'lucide-react'
...
import type { UserSkill } from '../model/types'
...
// ... (skipping imports and schema until AddRoleModal component)
...
export function AddRoleModal({ open, onClose, onSuccess }: AddRoleModalProps) {
    ...
    try {
      ...
      if (isOtherSelected && customSkillValue) {
        try {
          ...
          // If skill already exists or fails, we continue but ignore the error
          console.warn('Failed to auto-save new skill:', customSkillValue)
        }
      }
      ...
      await userApi.createUser({
        ...
        // Send skills that are either hardcoded or already known dynamic
        skills: data.skills.filter(s => s !== 'OTHER') as UserSkill[],
        customSkills: customSkillValue,
      })
  ...
  const combinedSkillsList = useMemo(() => {
    // Merge hardcoded skills with skills from DB
  ...
  return (
    <Modal
      ...
      subtitle="Organizational Onboarding"
      icon={<UserPlus size={22} className="text-primary-olive" />}
      footer={
    ...
    >
      <form id="add-role-form" onSubmit={handleSubmit(onSubmit)} className="space-y-10">
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            />
            <Input label="System Password" type="password" required placeholder="••••••••" error={errors.password?.message} {...register('password')} />
          </div>
          ...
          <div className="space-y-5">
            ...
            <div className="flex flex-wrap gap-2.5">
              {combinedSkillsList.map(skill => {
                const isOther = skill.value === 'OTHER'
            ...
            {isOtherSelected && (
              <div className="p-5 bg-primary-50 rounded-3xl border border-primary-olive/20 animate-in fade-in slide-in-from-top-2 duration-300">
                <div className="flex flex-col gap-3">
                  ...
                  <p className="text-[9px] font-medium text-[#6B7280] px-1 italic">
                    Note: This skill will be automatically saved to the system catalog for future selection.
                  </p>
                </div>
              </div>
            )}
