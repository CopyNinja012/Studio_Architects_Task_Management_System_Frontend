...
import { Plus, Search, Building2, X, FolderKanban, Calendar, MapPin } from 'lucide-react'
...
import { PROJECT_STATUSES_LIST, PROJECT_TYPES } from '@/features/admin/model/constant'
import { useAuthStore } from '@/store'

// ─── Project Form Schema ──────────────────────────────────────────────────────
// ... (rest of the file content until PMProjects function)
...
function CreateProjectModal({ open, onClose, onCreated }: { open: boolean; onClose: () => void; onCreated: () => void }) {
  ...
  const onSubmit = async (values: ProjectFormValues) => {
    try {
      ...
      onCreated()
      onClose()
      navigate(PATHS.PM_PROJECT_DETAIL.replace(':id', created.id))
    } catch (err: any) {
  ...
  return (
    <Modal
      ...
      icon={<FolderKanban size={22} className="text-primary-olive" />}
      footer={
        <div className="flex items-center gap-3">
          ...
          <Button 
            ...
            className="bg-primary-olive hover:bg-[#4B5A2A] text-white rounded-xl px-8 font-black shadow-lg shadow-primary-olive/20 transition-all hover:-translate-y-1"
            icon={<Plus size={16} />}
          >
            Create Project
          </Button>
      ...
      <form id="create-project-form" onSubmit={handleSubmit(onSubmit)} className="space-y-10">
        ...
        <div className="space-y-6">
          <div className="flex items-center gap-3 border-b border-[#E5E7EB] pb-3">
            <span className="w-6 h-6 rounded-lg bg-[#F8FAF5] flex items-center justify-center text-primary-olive border border-[#E5E7EB]">
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
            <Controller name="projectLeadId" control={control} render={({ field }) => (
              <Dropdown label="Project Manager" searchable options={managers} value={field.value ?? ''} onChange={field.onChange} placeholder={loadingPeople ? 'Loading…' : 'Select lead'} error={errors.projectLeadId?.message} />
            )} />
            <Controller name="assignedEmployeeId" control={control} render={({ field }) => (
              <Dropdown label="Assigned Employee" searchable options={employees} value={field.value ?? ''} onChange={field.onChange} placeholder={loadingPeople ? 'Loading…' : 'Select member'} error={errors.assignedEmployeeId?.message} />
            )} />
...
function StatusChip({ status }: { status: ProjectStatus }) {
  return (
    <span className={cn('inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-bold border', STATUS_STYLES[status])}>
      {status.replace('_', ' ')}
    </span>
  )
}
...
import { ProjectDetailModal } from '@/features/admin/components/ProjectDetailModal'
...
export default function PMProjects() {
  ...
  useEffect(() => { fetchProjects() }, [page, pageSize, statusFilter, typeFilter, user?.id])
  ...
  const columns: Column<ProjectResponse>[] = [
    ...
    { key: 'jobNumber', header: 'Job Number', sortable: true, render: row => <span className="text-[12px] font-semibold text-text-medium">{row.jobNumber}</span> },
    {
    ...
    { key: 'projectType', header: 'Project Type', align: 'center', render: row => <TypeBadge type={row.projectType} /> },
    { key: 'status', header: 'Status', align: 'center', render: row => <StatusPill status={row.status} /> },
    { key: 'startDate', header: 'Start Date', sortable: true, render: row => <span className="text-[12px] font-semibold text-text-medium">{fmtDate(row.startDate)}</span> },
    { key: 'expectedCompletionDate', header: 'Deadline', sortable: true, render: row => <DeadlineCell date={row.expectedCompletionDate} /> },
    { key: 'progress', header: 'Progress', render: row => <ProgressCell value={row.progress} /> },
  ]
